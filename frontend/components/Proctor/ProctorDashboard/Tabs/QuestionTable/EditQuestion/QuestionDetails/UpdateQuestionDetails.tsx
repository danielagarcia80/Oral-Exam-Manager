    ...
    if (window.confirm('Are you sure you want to delete this question?')) {
      try {
        const response = await fetch(
          `http://localhost:3001/questions/${currentQuestion.question_id}`,
          {
            method: 'DELETE',
          }
        );
  ...
  useEffect(() => {
    const fetchKeywords = async () => {
      if (currentQuestion && currentQuestion.question_id) {
        try {
          const response = await fetch(
            `http://localhost:3001/questions/${currentQuestion.question_id}/keywords`
          );
          if (!response.ok) {
      ...
      <Box maw={800} mx="auto">
        <form onSubmit={form.onSubmit(handleUpdateQuestion)}>
          ...
          <Group mt="md">
            ...
            <Button type="submit" color="green" size="xl" style={{ margin: '10px' }}>
              Update Question
            </Button>
          </Group>
        </form>

...
export default function CodeBlock() {
  ...
  const handleQuestionChange = () => {
    ...
    if (
      currentQuestion.question_type === 'context' &&
      currentQuestion.context_keywords?.length > 0
    ) {
      console.log('Finding context lines...', currentQuestion.context_keywords[0].keyword);
      const { contextLines, keywordLine } = findContextAndKeyword(
        fileString,
        currentQuestion.context_keywords[0].keyword,
        currentQuestion.keywords[0].keyword
      );
    ...
    return {
      ...
      keywordLine:
        keywordLines.length > 0
          ? keywordLines[Math.floor(Math.random() * keywordLines.length)]
          : null,
    };
  }

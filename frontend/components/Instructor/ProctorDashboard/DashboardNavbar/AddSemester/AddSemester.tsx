...
import { Button, Group, Select, Divider } from "@mantine/core";
...
const AddSemesterModal: React.FC<Props> = ({ closeModal }) => {
    ...
    const handleAddSemester = async () => {
        try {
            ...
            const highestSemesterId = Math.max(...semesters.map((semester: any) => parseInt(semester.semester_id, 10)));
            ...
            if (addSemesterResponse.ok) {
                ...
                const coursesResponse = await fetch(`http://localhost:3001/courses/previous-semester/${highestSemesterId}`);
                const courses = await coursesResponse.json();
                console.log('addsemester:',courses);
                setPreviousSemesterCourses(courses);
    
                // Fetch teachers from the previous semester
                const teachersResponse = await fetch(`http://localhost:3001/teachers/previous-semester/${highestSemesterId}`);
